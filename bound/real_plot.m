...
if 1,
	...
	for i = 1:npoints,
		...
		C2(i,:) = prod(C2x);
		[C1x, C1_2x, C0x, Pix, leps{i}, ll{i}] = tighter_bound(u1, u2, points(i,:), values, prms);
		C0(i,:) = prod(prod(C0x));
		C1(i,:) = prod(prod(C1x));
...
% load previously computed results because it is faster
load('P_new.mat')
...
figure
hold on;
semilogx([0:prms.iters],squeeze(Pt0(end,:,:,1)),'.-')
semilogx([0:prms.iters],squeeze(Pt1(end,:,:,1)),':')
semilogx([0:prms.iters],squeeze(Pt2(end,:,:,1)),'.')
semilogx([0:prms.iters],squeeze(Pt3(end,:,:,1)),'.')
% 
% figure
% hold on
...
figure
hold on
h=plot([0:prms.iters],squeeze(P(1,:,1:3,1)))
set(gca,'fontsize',16);
line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,1)), 20, '*-', 'spacing', 'curve', 'color', blue)
line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,2)), 20, '*-', 'spacing', 'curve', 'color', red)
line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,3)), 20, '*-', 'spacing', 'curve', 'color', yellow)

line_fewer_markers([0:prms.iters],squeeze(Pt0(1,:,1)), 20, 'o-', 'spacing', 'curve', 'color', blue)
line_fewer_markers([0:prms.iters],squeeze(Pt0(1,:,2)), 20, 'o-', 'spacing', 'curve', 'color', red)
line_fewer_markers([0:prms.iters],squeeze(Pt0(1,:,3)), 20, 'o-', 'spacing', 'curve', 'color', yellow)
h=legend(h, {'\epsilon=1','\epsilon=2','\epsilon=5'})
set(h,'FontSize',16);
grid on
box on
...
h=plot([0:prms.iters],squeeze(P(1,:,1:3,1)))
set(gca,'fontsize',16);
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(1,:,1,1)), 20, 'd-', 'spacing', 'curve', 'color', blue)
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(1,:,2,1)), 20, 'd-', 'spacing', 'curve', 'color', red)
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(1,:,3,1)), 20, 'd-', 'spacing', 'curve', 'color', yellow)

line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,1,1)), 20, '*-', 'spacing', 'curve', 'color', blue)
line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,2,1)), 20, '*-', 'spacing', 'curve', 'color', red)
line_fewer_markers([0:prms.iters],squeeze(Pt1(1,:,3,1)), 20, '*-', 'spacing', 'curve', 'color', yellow)
h=legend(h, {'\epsilon=1','\epsilon=2','\epsilon=5'})
set(h,'FontSize',16);
grid on
box on
...
figure
hold on
h=plot([0:prms.iters],squeeze(P(end,:,1:3,1)))
set(gca,'fontsize',16);
...
line_fewer_markers([0:prms.iters],squeeze(Pt0(end,:,3,1)), 20, 'o-', 'spacing', 'curve', 'color', yellow)
h=legend(h, {'\epsilon=1','\epsilon=2','\epsilon=5'})
set(h,'FontSize',16);
grid on
box on
...
h=plot([0:prms.iters],squeeze(P(end,:,1:3,1)))
set(gca,'fontsize',16);
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(end,:,1,1)), 20, 'd-', 'spacing', 'curve', 'color', blue)
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(end,:,2,1)), 20, 'd-', 'spacing', 'curve', 'color', red)
line_fewer_markers([0:prms.iters],squeeze(Pt1_2(end,:,3,1)), 20, 'd-', 'spacing', 'curve', 'color', yellow)
...
%define a color scheme for the following
C = [0 15];
%% show the different values of epsilon for two points

figure;
h=imagesc(leps{1}(:,:,1), C);
set(gca,'fontsize',18);
figure;
imagesc(leps{1}(:,:,5), C);
set(gca,'fontsize',18);
